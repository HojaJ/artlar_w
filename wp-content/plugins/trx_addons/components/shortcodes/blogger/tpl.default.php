...
if ( (int)$query->post_count > 0 || count($tabs) > 0 ) {
	...
	if (!empty($args['columns_tablet'])) $args['columns_tablet'] = (int)$posts_in_grid > 0 ? max(1, min(3, (int) $args['columns_tablet'])) : max(1, min(12, (int) $args['columns_tablet']));
	if (!empty($args['columns_mobile'])) $args['columns_mobile'] = (int)$posts_in_grid > 0 ? max(1, min(3, (int) $args['columns_mobile'])) : max(1, min(12, (int) $args['columns_mobile']));
	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		if ($args['slider']) {
			...
			?><div class="sc_blogger_content sc_item_content sc_item_posts_container<?php
				...
				if (!empty($args['full_post']) && (int)$args['columns'] == 1) echo ' open_full_post';
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			if ( !apply_filters('trx_addons_filter_sc_blogger_template', false, $args) ) {
				$args['item_number'] += !trx_addons_is_off($args['pagination']) && (int)$args['page'] > 1 ? ( $args['page'] - 1 ) * $args['count'] : 0;
